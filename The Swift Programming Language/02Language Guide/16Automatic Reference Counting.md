...
然而，我们有可能会写出这样的代码，一个类的实例永远不会有0个强应用。在两个类实例彼此保持对方的强引用，使得每个实例都使对方保持有效时会发生这种情况。我们称之为*强引用环*。
...
只有`number73`还持有`Apartment`实例的强引用。如果你破坏这个强引用，那么也不存在`Apartment`实例的任何强应用：
...
接下来的例子定义了两个类，`Customer`和`CreditCard`，模拟了银行客户和客户的信用卡。每个类都一个属性，存储另外一个类的实例。这样的关系可能会产生强应用环。
