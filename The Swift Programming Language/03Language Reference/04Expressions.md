...
前缀表达在表达式的前面加入一个前缀运算符。前缀运算符后面紧跟表达式，这个表达式可以理解为前缀运算符的参数。
...
这些运算符的使用方法请参考[Basic Operators]() 和 [Advanced Operators]() 相关章节。
...
* 如果这种向特定类型的转换成功，那么这个表达式将被返回此特定类型的一个实例。典型的例子是向上转型。
...
在构造器，子脚本或实例方法中，`self`指当前实例的引用。在静态方法或类方法中，`self`指当前类的引用。

`self`用于当同一作用域存在同名变量的时候来明确指代以访问成员变量，例如临时变量与成员变量同名的情况：
...
闭包创建一个闭合区间，在其他语言中，也被称为lambda或匿名函数。像方法声明一样，闭包也包含供其执行的语句并从此闭合区间取值。它的形式如下：
...
* 闭包可以不写它的参数类型，返回值类型。如果省略参数名和其类型，同时省略语句前的`in`关键字；如果无法推断其类型，则会产生编译时错误。
* 如果闭包省略了它的参数名，它的参数将会隐式的以$加其位置编号命名：$0,$1,$2等等。
* 只有一个表达式的闭包默认返回表达式的值。当周围有表达式操作引用类型时，这个表达式的内容也会参与其中。下面的几个闭包是等价的:
...
使用抓取列表（capture list），闭包可以显式的得到特定值。抓取列表内容由中括号括起，由逗号分隔，后面是参数列表。当你使用抓取列表时，即使你省略参数名，参数类型和返回值类型，你也必须写`in`关键字。
...
利用括号表达式我们可以为函数调用传参。如果括号内只有一个值，其括号表达式的类型即为此值的类型。例如（1）的类型是`Int`，不是`(Int)`。
...
```
// someFunction 的参数列表由一个参数x和一个闭包组成
someFunction(x, {$0 == 13})
someFunction(x) {$0 == 13}
```
...
亚脚本表达式使用对应声明的get方法个set方法来获取亚脚本。它的格式如下：
...
如果自判断链接是嵌套的，只返回最外层的的可选值类型。在下面的例子中，如果`c`不是nil，它的值将被拆包，并用以获得`.property` 和 `.performAction()`的值。整个`c?.property.performAction() `是可选值类型。
