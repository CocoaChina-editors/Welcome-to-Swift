...
C 语言作为与 Objective-C 语言的互操作性的一部分，Swift 也兼容一些与C语言的类型和特性，如果你的代码有需要，Swift 也提供了和常见的 C 语言结构混合编程的编程方式。

## 基本数据类型

Swift 提供了一些和 C 语言的基本类型如*char*,*int*,*float*，*double*等价的Swift基本数据类型。然而，这些Swift的核心基本类型之间不能隐式相互转换，如Int。[因此，只有你的代码明确要求他们时再使用这些类型，而Int可以在任何你想使用它的时候使用。]
